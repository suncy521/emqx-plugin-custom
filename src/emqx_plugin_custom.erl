...
on_client_authenticate(Credentials = #{client_id := ClientId, username := Username, password := Password}, _Env) ->
  io:format("Client(~s) authenticate, Password:~p ~n", [ClientId, Username, Password]),
  {ok, Credentials#{auth_result => success}}.
